...
text(20, 40, paste("AUC=", auc1), col="blue")
text(20, 35, paste("AUC=", auc2), col="red")
text(20, 30, paste("AUC=", auc3), col="green")
...
```{r}
mod1_iter <- glm(save ~ idade*factor(status), 
            data = amostra[,-1], family=binomial(link="logit")) 
summary(mod1_iter)

mod2_iter <- glm(save ~ idade + factor(setor) +
                   factor(status) + factor(setor):factor(status), 
                 data = amostra[,-1], 
...
```{r}
## MODELO 1
res <- exp(mod1$residuals)/(1+exp(mod1$residuals))
#gráficos residuais
...
```{r}
data.frame(real = mod1$data$save, 
           var1=amostra$idade, 
           var2=amostra$status, 
           pred=mod1$fitted.values) %>% 
  mutate(previsao=ifelse(pred>0.7, 1, 0),
         residual= real-previsao) %>% 
...
# Assuming you have a dataset named 'my_data' with columns 'y', 'x1', and 'x2'
# Fit the logistic regression model
model <- glm(vs ~ mpg + cyl + wt, data = mtcars, family = "binomial")

# Extract residuals
model_residuals <- residuals(model, type = "response")

ggplot(data.frame(residuals = model_residuals), aes(x = residuals)) +
  geom_histogram(binwidth = 0.1, fill = "lightblue", color = "black") +
  labs(title = "Histogram of Residuals", x = "Residuals", y = "Frequency")

ggplot(data.frame(fitted = fitted(model), residuals = model_residuals), aes(x = fitted, y = residuals)) +
  geom_point() +
